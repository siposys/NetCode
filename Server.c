...
static void *read_send(void *ptr) {
    ...
    char packet[32767];
	char c[32767];
	//uLong ucompSize = strlen(packet)+1;
	//uLong ucompSize = strlen(packet);
    ...
    do {
		...
		uncompress((Bytef *)c, &compSize, (Bytef *)packet, length);
        //printf("read %d bytes from interface.\n", length);
        if ((length = send(socket, c, compSize, MSG_NOSIGNAL)) == -1) {
            perror("send");
            break;
        }
