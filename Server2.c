...
static void *read_send(void *ptr) {
    ...
    do {
        ...
        //Encrypt before send - packet

        if ((length = send(socket, encryptDecrypt(packet), length, MSG_NOSIGNAL)) == -1) {
            perror("send");
            break;
        }
...
static void *recv_write(void *ptr) {
    ...
    do {
        ...
        length = recv(socket, encryptDecrypt(packet), sizeof(encryptDecrypt(packet)), 0);//changes
        if (length == 0) {
...
//Encrypttion-decryption using xor
static char[] encryptDecrypt(char inpString[])
{
    // Define XOR key
...
int main(int argc, char *argv[])
{
    ...
    // Wait for a tunnel.
    int tunnel;
    while ((tunnel = get_tunnel((char*)"8000", (char*)"test")) != -1) {
